...
const EditOwnerProfileModal = ({ isOpen, onClose, className }: EditOwnerProfileModalProps) => {
  ...
  const fields: Field[] = [
    ...
    { name: "phoneNumber", label: "사장님 전화번호", postPosition: "를", required: false },
    {
      ...
      postPosition: "를",
      required: true,
      icon: <FiMapPin className="text-grayscale-400 h-5 w-5" />,
    },
  ];
      ...
      >
              ...
              <div className="relative h-[110px] w-[110px] md:h-[120px] md:w-[120px]">
                ...
                >
                  {previewUrl ? (
                    ...
                    <FiUser className="text-grayscale-200 h-full w-full p-6" />
                  )}
                </button>
                <button
                  type="button"
        ...
        <div className="flex h-[58px] justify-between gap-3 md:h-[72px]">
          <button
            ...
            onClick={onClose}
            disabled={isSubmitting}
            className="border-grayscale-300 text-grayscale-700 hover:bg-grayscale-50 w-[158px] rounded-md border bg-white px-4 py-2 text-sm font-semibold transition-colors md:w-[314px] md:text-base"
          >
            취소
          </button>
          <button
