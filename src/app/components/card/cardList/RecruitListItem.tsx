...
const RecruitListItem = ({
  ...
  return (
    <div className="border-grayscale-200 relative h-auto w-full overflow-hidden rounded-xl border bg-white shadow-md transition-transform duration-300 hover:scale-[1.02] sm:h-[390px] sm:w-[327px] md:h-[536px] md:w-[477px]">
      {/* 이미지 슬라이더 영역 */}
      ...
      <div className="relative space-y-4 p-4 sm:p-4 md:p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="text-grayscale-500 flex flex-col gap-1 text-sm md:text-base">
            <div className="flex items-center gap-2">
              ...
              <span className="text-grayscale-500 hidden font-medium md:inline">
                {formatRecruitDate(recruitmentStartDate, true)} ~ {formatRecruitDate(recruitmentEndDate, true)}
              </span>
              <span className="text-grayscale-500 font-medium md:hidden">
          ...
          <div ref={dropdownRef} className="relative">
            <button
              className="text-grayscale-500 hover:text-grayscale-700"
            ...
            {/* 드롭다운 메뉴 */}
            {showDropdown && (
              <div className="border-grayscale-200 absolute right-0 top-8 z-10 w-32 rounded-lg border bg-white py-2 shadow-lg">
        ...
        {/* 위치 정보 */}
        <p className="text-grayscale-500 truncate text-sm md:text-base">{location}</p>

        {/* 통계 정보 (지원자, 스크랩, D-day) */}
        <div className="border-grayscale-100 text-grayscale-700 flex h-[50px] items-center justify-between rounded-2xl border px-4 text-sm md:text-base">
          <div className="flex flex-1 items-center justify-center">
            <span className="font-medium">지원자 {applyCount}명</span>
          </div>
          <div className="bg-grayscale-200/50 h-5 w-[1px]" />
          <div className="flex flex-1 items-center justify-center">
